...
namespace GentlemansV2
{
    ...
    public partial class MarcacoesEsteticista : TabbedPage
    {
        ...
        public ObservableCollection<MarcEsteticista> olds = new ObservableCollection<MarcEsteticista>();
        public ObservableCollection<MarcEsteticista> gets = new ObservableCollection<MarcEsteticista>();
        public ObservableCollection<MarcEsteticista> dayInfo = new ObservableCollection<MarcEsteticista>();

        public static MarcacoesEsteticista instance;

        public MarcacoesEsteticista()
        {
            InitializeComponent();
            instance = this;
            GetOld();
        ...
        async Task GetDayMarcacoes(DateTime _date)
        {
            ...
            var keyValues = new List<KeyValuePair<string, string>> {
                ...
                new KeyValuePair<string, string>("date", _date.ToString("dd/MM/yyyy"))
            };

            var request = new HttpRequestMessage(HttpMethod.Post, Configs.server + "/api/marcacoes/get/estetica/day");
            ...
            for (int i = 0; i < jobsJson.Count; i++)
            {
                ...
                dayInfo.Add(new MarcEsteticista() { idJob = (int)jobsJson[i]["id"], cadeira = jobsJson[i]["cadeira_id"], date = date.ToShortDateString() + " " + jobsJson[i]["hora_inicio"], dayMonth = day + " " + month, year = year, hour = jobsJson[i]["hora_inicio"] });
            }
        ...
        {
            Navigation.PushPopupAsync(new MarcarEsteticista());
        }

        async Task Get()
        {
            calendarView.SpecialDates.Clear();
            var request = new HttpRequestMessage(HttpMethod.Get, Configs.server + "/api/marcacoes/get/estetica/" + User.id);
            ...
            for (int i = 0; i < jobsJson.Count; i++)
            {
                ...
                gets.Add(new MarcEsteticista() { idJob = (int)jobsJson[i]["id"], _DATE = date, cadeira = jobsJson[i]["cadeira_id"], date = date.ToShortDateString(), hour = jobsJson[i]["hora_inicio"] });
            }
        ...
        async Task GetOld()
        {
            var request = new HttpRequestMessage(HttpMethod.Get, Configs.server + "/api/marcacoes/old/estetica/" + User.id);
            ...
            for (int i = 0; i < jobsJson.Count; i++)
            {
                ...
                olds.Add(new MarcEsteticista() { idJob = (int)jobsJson[i]["id"], imageURL = jobsJson[i]["image"], cadeira = jobsJson[i]["cadeira_id"], date = date.ToShortDateString() + " " + jobsJson[i]["hora_inicio"], dayMonth = day + " " + month, year = year, hour = jobsJson[i]["hora_inicio"] });
            }
        ...
        void Photo_Click(object sender, System.EventArgs e)
        {
            ...
            Navigation.PushAsync(new ImageFinishEstetica(grid.ClassId));
        }

        void Marcacao_Tapped(object sender, System.EventArgs e)
        {
            Grid _grid = (Grid)sender;
            Navigation.PushPopupAsync(new InfoMarcacaoEstetica(_grid.ClassId));
        }
    }
}